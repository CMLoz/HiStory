...
// ──────────────────────────────────────────────
// Luna-specific providers
// ──────────────────────────────────────────────

final lunaChapterProvider = NotifierProvider<LunaChapterNotifier, String>(
  LunaChapterNotifier.new,
);

class LunaChapterNotifier extends Notifier<String> {
  @override
  String build() {
    return 'chapter1';
  }
...
final lunaDialogueNodesProvider = FutureProvider<Chapter>((ref) async {
  final chapterId = ref.watch(lunaChapterProvider);
  final jsonString = await rootBundle.loadString(
    'assets/dialogues/luna/$chapterId.json',
  );
  final dynamic jsonData = jsonDecode(jsonString);
...
class LunaGameStateNotifier extends Notifier<int> {
  @override
  int build() {
    return 0;
  }
...
final lunaGameStateProvider = NotifierProvider<LunaGameStateNotifier, int>(() {
  return LunaGameStateNotifier();
});
